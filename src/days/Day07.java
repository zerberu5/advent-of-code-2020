...
public class Day07 {
    ...
    public static void main(String[] args) {
        ...
        String unformattedBagRules2 = "dim red bags contain 2 bright gold bags, 5 striped fuchsia bags.\n" +
        ...
        String unformattedBagRules = "light red bags contain 1 bright white bag, 2 muted yellow bags.\n" +
                "dark orange bags contain 3 bright white bags, 4 muted yellow bags.\n" +
                "bright white bags contain 1 shiny gold bag.\n" +
                "muted yellow bags contain 9 shiny gold bags, 2 faded blue bags.\n" +
        ...
        if (goldBag == null) {
            ...
            int count = getCountOfAllGoldChildren(goldBag) - 1;
            System.out.println(count);
        }
    }

    private static int getCountOfAllGoldChildren(Bag goldBag) {
        int count = 1;
        for (Map.Entry<Bag, Integer> entry : goldBag.getInnerBags().entrySet()) {
            Bag bag = entry.getKey();
            int bagCount = entry.getValue();
            count += getCountOfAllGoldChildren(bag) * bagCount;
        }
        return count;
    }
    ...
    private static List<Bag> getGoldGrandParents(List<Bag> allBags, List<Bag> goldParents, int added) {
        if (added > 0) {
            added = 0;
            for (Bag bag : allBags) {
                if (bag.getInnerBags().keySet().stream().anyMatch(goldParents::contains) && !goldParents.contains(bag)) {
                    goldParents.add(bag);
                    added++;
                }
            }
            getGoldGrandParents(allBags, goldParents, added);
        }
    ...
    private static List<Bag> getDirectParentsOfGold(List<Bag> bags) {
        ...
        for (Bag bag : bags) {
            for (Bag innerBag : bag.getInnerBags().keySet()) {
                if (innerBag.getColor().equals("shiny gold")) {
                    goldParents.add(bag);
                }
            }
        }
    ...
    private static List<Bag> createInnerBags(List<String> bagsText) {
        List<Bag> bags = new ArrayList<>();
        for (String bagText : bagsText) {
            Bag bag = new Bag();
            bag.setColor(bagText.split(" bags contain")[0]);

            List<Integer> counts = getCounts(bagText);
            List<String> innerBagColors = getInnerBagColors(bagText);

            for (int i = 0; i < counts.size(); i++) {
                bag.getInnerBags().put(new Bag(innerBagColors.get(i)), counts.get(i));
            }
            bags.add(bag);
        }
        return bags;
    ...
    private static List<String> getInnerBagColors(String bagRuleText) {
        List<String> innerBagColors = new ArrayList<>();

        bagRuleText = bagRuleText.substring(bagRuleText.indexOf("contain ") + 8);

        String[] allCountBag = bagRuleText.split(", ");

        for (String countBag : allCountBag) {
            countBag = countBag.replace("bags", "");
            countBag = countBag.replace("bag", "");
            countBag = countBag.replace(countBag.substring(0, 2), "");
            countBag = countBag.replace(".", "");
            countBag = countBag.replace("no other", "");
            innerBagColors.add(countBag.substring(0, countBag.length() - 1));
        }
        return innerBagColors;
    }

    private static List<Integer> getCounts(String bagRuleText) {
        List<Integer> counts = new ArrayList<>();

        for (int i = 0; i < bagRuleText.length(); i++) {
            // Decimal Codes of chars, whereas 49 = 0 and 57 = 9
            if (bagRuleText.charAt(i) >= 49 && bagRuleText.charAt(i) <= 57) {
                counts.add(Integer.parseInt(String.valueOf(bagRuleText.charAt(i))));
            }
        }
        return counts;
    }
...
class Bag {
    ...
    public boolean equals(Object o) {
        ...
        return ((Bag) o).getColor().equals(this.getColor());
    }
